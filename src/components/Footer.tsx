...
export function Footer({ language, showAds, onPrivacyClick, onImprintClick, onCookieSettingsClick }: FooterProps) {
  ...
  return (
    <footer className={`fixed bottom-0 left-0 right-0 ${!showAds ? 'bg-gradient-to-r from-amber-50 to-yellow-50 border-amber-200' : 'bg-white/80'} backdrop-blur-sm border-t py-2 px-4 z-40`}>
      <div className="max-w-6xl mx-auto flex flex-wrap items-center justify-between gap-2 text-sm">
        <div className="flex items-center gap-4">
          <span className={!showAds ? 'text-amber-700 font-medium' : 'text-gray-500'}>v1.6.0</span>
          {!showAds && (
